...
int main( int argc, char **argv) {
  ...
  curContext = tlGetRenderContext(0,
                                  tlGethWnd(),
  ...
  grGet(GR_GLIDE_STATE_SIZE, 4, (FxI32 *)&state_size);
...
#if 1
    triDecal.largeLodLog2 = GR_LOD_LOG2_64;
    triDecal.data = ((FxU8*)triDecal.data + 
                     (sizeof(FxU16) * ((256 * 256) + (128 * 128))));
#endif
  ...
  while ( 1 ) {
    ...
    curContext = tlCheckRenderContext(0);
    if (curContext == NULL) {
      resetStateP = FXTRUE;
    } else {
      if (resetStateP) {
      ...
      if (render == FXTRUE) {
        ...
        if (plugging) 
          grSplash(0.f, (origin == GR_ORIGIN_UPPER_LEFT) ? 4.0f / 5.0f * scrHeight : 0.f, 
                   scrWidth / 5.f, scrHeight / 5.f, 
                   frameNum);
		...
		/*  Fix for PRS 8197
		    Delay between pressing a key and "action" happening when
			rendering to the front buffer.  The hware seems to buffer up
			a number of frames so we call grFinish() to remove this delay. jmccartney 13/04/00
		*/
		else
		  grFinish();

        if (!grSelectContext(*curContext)) *curContext = 0x00UL;
      }
    }
